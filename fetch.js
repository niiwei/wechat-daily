...
function getAccountName(feedUrl, feed) {
  ...
  if (match && nameMap[match[1]]) {
    return nameMap[match[1]];
  }
  
  return feed.title || '未知公众号';
}
...
async function main() {
    ...
    feed.items.forEach(item => {
      ...
      if (pubDate && isYesterday(pubDate)) {
        yesterdayArticles.push({
          ...
          title: item.title,
          link: item.link,
          summary: extractCoreContent(item['content:encoded'] || item.content || item.contentSnippet),
  ...
  console.log(`找到 ${yesterdayArticles.length} 篇昨天发布的文章`);
